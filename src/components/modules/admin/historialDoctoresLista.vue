<template>
  <div class="pdf-container">
    ...
    <div class="text-center q-mb-lg header-title">
      <h5 class="text-primary no-padding no-margin text-bold">
        LISTADO DE DOCTORES {{ isActive ? 'ACTIVOS' : 'INACTIVOS' }}
      </h5>
      <p class="text-grey-7 no-padding q-mt-xs">Sistema de Gestión Médica - CDI</p>
    </div>

    <!-- LISTADO DE DOCTORES -->
    <div class="info-section">
      <div :class="['section-title', isActive ? 'bg-active' : 'bg-inactive']">
        <span>DOCTORES {{ isActive ? 'ACTIVOS' : 'INACTIVOS' }} EN EL SISTEMA</span>
        <span class="count-badge">Total: {{ displayList.length }}</span>
      </div>
      ...
      <div class="table-wrapper">
        <table class="styled-table">
          ...
          <tbody>
            <tr v-for="(doctor, index) in displayList" :key="'dr-' + index">
            ...
            <tr v-if="displayList.length === 0">
              <td colspan="6" class="text-center q-pa-md text-grey">
                No se encontraron doctores {{ isActive ? 'activos' : 'inactivos' }} registrados.
              </td>
            </tr>
          </tbody>
        </table>
  ...
  computed: {
    displayList() {
      ...
      return this.data.filter(d => {
        const e = (d.usuarios && d.usuarios.estado ? d.usuarios.estado : '').toString().toLowerCase();
        const isCurrentlyActive = (e === 'activo' || e === 'act' || e === '1' || e === 'true' || e === 'si' || e === 'sí');
        return this.isActive ? isCurrentlyActive : !isCurrentlyActive;
      });
    }
  },
  methods: {
...
.bg-active { background-color: #2e7d32; }
.bg-inactive { background-color: #c62828; }

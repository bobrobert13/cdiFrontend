  ...
  <div style="margin: 12px 12px">
    ...
    <q-markup-table >
        ...
        <tr>
          <th colspan="6">
            <div class="row no-wrap items-center">
              <div class="text-bold text-white">Paciente: {{ data.persona.nombre1 }}</div>
            </div>
          </th>
        </tr>
        <tr class="text-white">
      ...
      <tbody class="bg-grey-3">
        <tr>
          <td class="text-right">{{ data.persona.nacionalidad }}</td>
          <td class="text-right">{{ data.persona.cedula_identidad }}</td>
          <td class="text-right">{{ data.persona.edad }}</td>
          <td class="text-right">{{ data.persona.estado_civil }}</td>
          <td class="text-right">{{ data.persona.sexo }}</td>
          <td class="text-right">{{ entradaFecha(data.createdAt) }}</td>
        </tr>
      </tbody>
    </q-markup-table>
    ...
    <q-markup-table  >
      <thead class="bg-primary no-border no-border-radius">
        <tr>
          <th colspan="6">
            <div class="row no-wrap items-center">
              <div class="text-bold text-white">Información de contacto</div>
            </div>
          </th>
        </tr>
        <tr class="text-white">
    ...
    <!-- <q-markup-table flat bordered class="q-mt-md">
      ...
      <tbody :class="$q.dark.isActive ? 'bg-grey-9' : 'bg-grey-3'">
        <tr v-for="(consulta, index) in data.consultas" :key="index" >
          <td class="text-left"><b>Diagnostico:</b> {{consulta.diagnostico}} / <b>Entrada</b>: {{ entradaFecha(consulta.ingreso) }} / <b>Salida</b>: {{ salidaFecha(consulta.salida) }}</td>
        </tr>
      </tbody>
    </q-markup-table> -->
  </div>

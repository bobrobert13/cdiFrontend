  ...
  <div v-if="data.examenes && data.examenes.length > 0" class="info-section">
      ...
      <div class="compact-table">
        <div class="table-header" style="grid-template-columns: repeat(5, 1fr);">
          ...
          <div class="header-cell">Resultados</div>
          <div class="header-cell">Estado</div>
          <div class="header-cell">Laboratorio</div>
          <div class="header-cell">Fecha</div>
        </div>

        <div v-for="(ex, idx) in data.examenes" :key="idx">
          <div class="table-row" style="grid-template-columns: repeat(5, 1fr);">
            <div class="table-cell">{{ ex.tipo_de_examen || 'No especificado' }}</div>
            <div class="table-cell">{{ ex.resultados || 'No especificado' }}</div>
            <div class="table-cell">{{ ex.estado_examen || 'No especificado' }}</div>
            <div class="table-cell">{{ ex.laboratorio_centro || 'No especificado' }}</div>
            <div class="table-cell">{{ ex.createdAt ? entradaFecha(ex.createdAt) : 'No especificado' }}</div>
          </div>

          <div class="table-row" style="grid-template-columns: 1fr;">
            <div class="table-cell">
              <strong>Descripción:</strong>
              <span> {{ ex.descripcion || 'No especificado' }} </span>
            </div>
          </div>
          ...
          <div class="table-row" style="grid-template-columns: repeat(3, 1fr);">
            <div class="table-cell">
              ...
              <div>{{ ex.medico_solicitante || 'No especificado' }}</div>
            </div>
            <div class="table-cell">
              <strong>Valores referencia:</strong>
              <div>{{ ex.valores_referencia || 'No especificado' }}</div>
            </div>
            <div class="table-cell">
              <strong>Observaciones:</strong>
              <div>{{ ex.observaciones && ex.observaciones.trim() !== '' ? ex.observaciones : 'No especificado' }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else class="info-section">
      ...
      <div class="no-data">El paciente no posee exámenes registrados</div>
    </div>
    </div>

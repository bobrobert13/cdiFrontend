  ...
  <div v-if="data.medicamentos && data.medicamentos.length > 0" class="info-section">
      ...
      <div class="compact-table">
        <div class="table-header" style="grid-template-columns: repeat(5, 1fr);">
          ...
          <div class="header-cell">Dosis / Vía</div>
          <div class="header-cell">Frecuencia</div>
          <div class="header-cell">Duración</div>
          <div class="header-cell">Inicio - Fin</div>
        </div>

        <div v-for="(m, idx) in data.medicamentos" :key="m.id_medicamento || idx">
          <div class="table-row" style="grid-template-columns: repeat(5, 1fr);">
            <div class="table-cell" :title="m.nombre || 'No especificado'">{{ m.nombre || 'No especificado' }}</div>
            <div class="table-cell" :title="(m.dosis || '') + ' / ' + (m.via_administracion || m.via || 'No especificado')">{{ (m.dosis || 'No especificado') + ' / ' + (m.via_administracion || m.via || 'No especificado') }}</div>
            <div class="table-cell" :title="m.frecuencia || 'No especificado'">{{ m.frecuencia || 'No especificado' }}</div>
            <div class="table-cell" :title="m.duracion || 'No especificado'">{{ m.duracion || 'No especificado' }}</div>
            <div class="table-cell">{{ m.fecha_inicio ? entradaFecha(m.fecha_inicio) : 'No especificado' }} - {{ m.fecha_fin ? entradaFecha(m.fecha_fin) : 'No especificado' }}</div>
          </div>

          <div class="table-row" style="grid-template-columns: 1fr;">
            <div class="table-cell">
              <strong>Principio activo:</strong>
              <span :title="m.principio_activo || 'No especificado'"> {{ m.principio_activo || 'No especificado' }} </span>
            </div>
          </div>
          ...
          <div class="table-row" style="grid-template-columns: repeat(3, 1fr);">
            <div class="table-cell">
              ...
              <div>{{ m.laboratorio || 'No especificado' }}</div>
            </div>
            <div class="table-cell">
              <strong>Tipo / Estado:</strong>
              <div>{{ (m.tipo_medicamento || 'No especificado') + ' / ' + (m.estado_tratamiento || m.estado || 'No especificado') }}</div>
            </div>
            <div class="table-cell">
              <strong>Observaciones / Contraindicaciones / Efectos:</strong>
              <div :title="[(m.observaciones || ''), (m.contraindicaciones || ''), (m.efectos_secundarios || '')].filter(Boolean).join('\n')">
                {{ shortText(((m.observaciones || '') + ' ' + (m.contraindicaciones || '') + ' ' + (m.efectos_secundarios || '')).trim(), 160) || 'No especificado' }}
              </div>
            </div>
          </div>
    ...
    <div v-else class="info-section">
      ...
      <div class="no-data">El paciente no posee medicamentos registrados</div>
    </div>

<template>
  <div class="pdf-container">
    ...
    <div class="info-section" v-if="data">
      ...
      <div class="info-grid">
        <div class="info-item">
          ...
          <span class="value">{{ data.reporte || 'Consultas por Período' }}</span>
        </div>
        <div class="info-item">
    ...
    <!-- DISTRIBUCIÓN POR PERÍODO (Semanal -> días, Mensual -> semanas) -->
    <div class="info-section" v-if="filasSemanas.length">
      <div class="section-title">{{ tituloDistribucion }}</div>
      <div class="compact-table">
        <div class="table-header semanal">
          <div class="header-cell">{{ etiquetaUnidad }}</div>
        ...
        <div v-for="(fila, i) in filasSemanas" :key="i" class="table-row semanal">
          <div class="table-cell">{{ fila.etiqueta }}</div>
    ...
    <div class="info-section" v-else>
      <div class="section-title">{{ tituloDistribucion }}</div>
      <p class="text-grey-6" style="padding:4px 8px; background:#f5f5f5; border-radius:3px;">No hay datos para el período seleccionado.</p>
    </div>
...
export default {
  ...
  computed: {
    ...
    filasSemanas() {
      if (!this.data || !Array.isArray(this.data.datos)) return [];
      const total = this.totalConsultas;
      const categoriasValidas = Array.isArray(this.data.categorias) && this.data.categorias.length === this.data.datos.length && this.data.categorias.length > 0;
      const esSemanal = this.data.periodo === 'week';
      const prefijo = esSemanal ? 'Día' : 'Semana';
      const categorias = categoriasValidas ? this.data.categorias : this.data.datos.map((_, i) => `${prefijo} ${i + 1}`);
      return categorias.map((etiqueta, idx) => {
        const valor = Number(this.data.datos[idx]) || 0;
        const porcentaje = total > 0 ? (valor / total) * 100 : 0;
        return { etiqueta, consultas: valor, porcentaje };
      });
    },
    tituloDistribucion() {
      if (!this.data) return 'Distribución';
      return this.data.periodo === 'week' ? 'Distribución Diaria' : 'Distribución Semanal';
    },
    etiquetaUnidad() {
      if (!this.data) return 'Unidad';
      return this.data.periodo === 'week' ? 'Día' : 'Semana';
    }
  },
  methods: {
    salidaFecha(salida) {

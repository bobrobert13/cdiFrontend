<template>
  <div class="pdf-container">
    ...
    <div class="info-section" v-if="data">
      ...
      <div class="info-grid">
        <div class="info-item">
          ...
          <span class="value">{{ data.reporte || 'Distribución por Edad' }}</span>
        </div>
        <div class="info-item">
        ...
        <div class="info-item" v-if="filasEdad.length">
          <span class="label">Grupo Mayor:</span>
          <span class="value">{{ grupoMayor.nombre }} ({{ grupoMayor.cantidad }})</span>
        </div>
        <div class="info-item" v-if="filasEdad.length">
          <span class="label">Promedio por Grupo:</span>
          <span class="value">{{ promedioGrupo.toFixed(1) }}</span>
        </div>
      </div>
    </div>

    <!-- DISTRIBUCIÓN POR GRUPO DE EDAD -->
    <div class="info-section" v-if="filasEdad.length">
      <div class="section-title">Distribución por Grupo de Edad</div>
      <div class="compact-table">
        ...
        <div v-for="(fila, i) in filasEdad" :key="i" class="table-row semanal">
        ...
        <div class="table-row semanal" v-if="data && filasEdad.length">
    ...
    <div class="info-section" v-else>
      <div class="section-title">Distribución por Grupo de Edad</div>
      <p class="text-grey-6" style="padding:4px 8px; background:#f5f5f5; border-radius:3px;">No hay datos de distribución por edad.</p>
    </div>
...
export default {
  ...
  computed: {
    ...
    filasEdad() {
      if (!this.data || !Array.isArray(this.data.datos) || !Array.isArray(this.data.categorias)) return [];
      const total = this.totalPacientes;
      const categorias = this.data.categorias;
      return categorias.map((nombre, idx) => {
    ...
    grupoMayor() {
      if (!this.filasEdad.length) return { nombre: 'N/D', cantidad: 0 };
      return this.filasEdad.reduce((max, cur) => cur.cantidad > max.cantidad ? cur : max);
    },
    promedioGrupo() {
      if (!this.filasEdad.length) return 0;
      return this.totalPacientes / this.filasEdad.length;
    }
  },
  methods: {
    salidaFecha(salida) {
